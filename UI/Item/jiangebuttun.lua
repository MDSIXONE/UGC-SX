---@class jiangebuttun_C:UUserWidget
---@field NewAnimation_1 UWidgetAnimation
---@field Button_0 UButton
--Edit Below--
local jiangebuttun = { bInitDoOnce = false }

function jiangebuttun:Construct()
    self:LuaInit()
end

function jiangebuttun:LuaInit()
...
function jiangebuttun:Button_0_OnClicked()
    local pc = UGCGameSystem.GetLocalPlayerController()
    if pc and pc.MMainUI then
        pc.MMainUI:ToggleJiange()
    end
end

function jiangebuttun:Button_0_OnHovered()
...
function jiangebuttun:Button_0_OnUnhovered()
    if self.NewAnimation_1 then
        if not self:IsAnimationPlaying(self.NewAnimation_1) then
...
function jiangebuttun:Button_0_OnPressed()
    if self.NewAnimation_1 then
        self:PlayAnimation(self.NewAnimation_1, 0, 1, 0, 2)
    end
end

function jiangebuttun:Button_0_OnReleased()
    if self.NewAnimation_1 then
