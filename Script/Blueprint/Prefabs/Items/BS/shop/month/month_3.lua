---@class month_3_C:Template_ItemHandle_C
--Edit Below--
local month_3 = {}

local BACKPACK_ITEM_ID = 8310184
local GIFT_PACK_ID = 312

function month_3:CanUseV2()
    return true
end

function month_3:OnUseV2()
    -- ugcprint("[month_3] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[month_3] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[month_3] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[month_3] 褰撳墠瀹濈锟斤拷锟介噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[month_3] 娌℃湁鍙敤") return end
