---@class month_6_C:Template_ItemHandle_C
--Edit Below--
local month_6 = {}

local BACKPACK_ITEM_ID = 8310187
local GIFT_PACK_ID = 315

function month_6:CanUseV2()
    return true
end

function month_6:OnUseV2()
    -- ugcprint("[month_6] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[month_6] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[month_6] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[month_6] 褰撳墠瀹濈鏁伴噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[month_6] 娌℃湁鍙敤") return end
