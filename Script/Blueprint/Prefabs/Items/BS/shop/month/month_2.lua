---@class month_2_C:Template_ItemHandle_C
--Edit Below--
local month_2 = {}

local BACKPACK_ITEM_ID = 8310183
local GIFT_PACK_ID = 311

function month_2:CanUseV2()
    return true
end

function month_2:OnUseV2()
    -- ugcprint("[month_2] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[month_2] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[month_2] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[month_2] 褰撳墠瀹濈鏁伴噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[month_2] 娌℃湁鍙敤") return end
