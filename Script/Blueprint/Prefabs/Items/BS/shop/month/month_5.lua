---@class month_5_C:Template_ItemHandle_C
--Edit Below--
local month_5 = {}

local BACKPACK_ITEM_ID = 8310186
local GIFT_PACK_ID = 314

function month_5:CanUseV2()
    return true
end

function month_5:OnUseV2()
    -- ugcprint("[month_5] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[month_5] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[month_5] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[month_5] 褰撳墠瀹濈鏁伴噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[month_5] 娌℃湁鍙敤") return end
