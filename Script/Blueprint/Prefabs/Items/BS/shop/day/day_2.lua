---@class day_2_C:Template_ItemHandle_C
--Edit Below--
local day_2 = {}

local BACKPACK_ITEM_ID = 8310180
local GIFT_PACK_ID = 302

function day_2:CanUseV2()
    return true
end

function day_2:OnUseV2()
    -- ugcprint("[day_2] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[day_2] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[day_2] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[day_2] 褰撳墠瀹濈鏁伴噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[day_2] 娌℃湁鍙敤") return end
