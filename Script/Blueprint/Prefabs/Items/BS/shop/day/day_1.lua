---@class day_1_C:Template_ItemHandle_C
--Edit Below--
local day_1 = {}

local BACKPACK_ITEM_ID = 8310179
local GIFT_PACK_ID = 301

function day_1:CanUseV2()
    return true
end

function day_1:OnUseV2()
    -- ugcprint("[day_1] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[day_1] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[day_1] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[day_1] 褰撳墠瀹濈鏁伴噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[day_1] 娌℃湁鍙敤") return end
