---@class YS_1_C:Template_ItemHandle_C
--Edit Below--
local YS_1 = {}

local BACKPACK_ITEM_ID = 8310167  -- 鑳屽寘鐗╁搧ID
local GIFT_PACK_ID = 331          -- 瀵瑰簲绀煎寘ID

function YS_1:CanUseV2()
    return true
end

function YS_1:OnUseV2()
    -- ugcprint("[YS_1] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[YS_1] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[YS_1] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[YS_1] 褰撳墠瀹濈鏁伴噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[YS_1] 娌℃湁瀹濈鍙敤") return end
