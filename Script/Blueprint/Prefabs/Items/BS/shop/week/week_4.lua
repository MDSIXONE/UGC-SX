---@class week_4_C:Template_ItemHandle_C
--Edit Below--
local week_4 = {}

local BACKPACK_ITEM_ID = 8310192
local GIFT_PACK_ID = 307

function week_4:CanUseV2()
    return true
end

function week_4:OnUseV2()
    -- ugcprint("[week_4] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[week_4] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[week_4] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[week_4] 褰撳墠瀹濈鏁伴噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[week_4] 娌℃湁鍙敤") return end
