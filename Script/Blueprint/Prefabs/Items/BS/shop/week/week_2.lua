---@class week_2_C:Template_ItemHandle_C
--Edit Below--
local week_2 = {}

local BACKPACK_ITEM_ID = 8310190
local GIFT_PACK_ID = 305

function week_2:CanUseV2()
    return true
end

function week_2:OnUseV2()
    -- ugcprint("[week_2] OnUseV2 瑙﹀彂")
    local BackpackComp = UGCItemSystemV2.GetOwnBackpackComponent(self)
    -- if not BackpackComp then ugcprint("[week_2] 閿欒锛氭棤娉曡幏鍙朆ackpackComponent") return end
    local PlayerController = BackpackComp:GetOwner()
    -- if not PlayerController then ugcprint("[week_2] 閿欒锛氭棤娉曡幏鍙朠layerController") return end
    ...
    -- ugcprint("[week_2] 褰撳墠瀹濈鏁伴噺: " .. tostring(ownedCount))
    -- if not ownedCount or ownedCount <= 0 then ugcprint("[week_2] 娌℃湁鍙敤") return end
